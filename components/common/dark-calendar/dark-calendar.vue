<template>
  <view class="sign-calendar">
    ...
    <image src="/static/images/dark-calendar/ico-arrow-up.png" mode="scaleToFill" @click="trgWeek()" class="weektoggel" :class="{ down: !monthOpen }"></image>
  </view>
</template>
  ...
  methods: {
    ...
    isSigned(y, m, d) {
      ...
      for (let i = 0; i < this.signeddates.length; i++) {
        let dy = dayjs(`${y}-${m}-${d}`).format('YYYY-MM-DD')
    ...
    selectOne(i, event) {
      ...
      let _data = dayjs(date).format('YYYY-MM-DD')
      let selectD = new Date(_data)
      if (selectD.getMonth() != this.m) {
        console.log('不在可选范围内')
        return false
      }
      this.choose = date
      this.$emit('on-click', _data)
    },
    // 上个月，下个月
    turning(_action) {
      if (_action === 'next') {
